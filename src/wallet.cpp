...
// Zeon: total coins staked (non-spendable until maturity)
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CZeonAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CZeonAddress(address).ToString());
}
...
// Zeon: check 'spent' consistency between wallet and txindex
// Zeon: fix wallet spent state according to txindex
...
// Zeon: disable transaction (only for coinstake)
